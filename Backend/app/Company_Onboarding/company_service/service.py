...
def if_super_admin(current_user: dict):
    return current_user.get("role") =="SUPER_ADMIN"

def onboarding( db: Session,current_user:dict,name: str,domain:str,subscription: SubscriptionEnum,admin_name: str,admin_last_name:str,admin_email: EmailStr,logo: str):
    try:
        if not if_super_admin(current_user):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="Only superadmin can oerform the onbaording")
        
        if get_company_by_name(db,name=name):
            raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE,
                                detail="Cpompany name alredy exists")
        
        if get_company_by_domain(db,domain=domain):
            raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE,
                                detail="Cpompany domain alredy exists")
    ...
    try:
        ...
        db.rollback()
        delete_otp(admin_email,redis_client)
        # raise
    return {
...
def edit_company(db: Session,company_id: int,name: str | None = None,domain: str | None = None,logo: str | None = None,subscription: SubscriptionEnum | None = None,):
    ...
    update_company_fields(db=db,company=company,name=name,domain=domain,logo=logo,subscription=subscription,)
    db.commit()
    db.refresh(company)
